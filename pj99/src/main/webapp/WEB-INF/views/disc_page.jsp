			...
			<div class="col-xs-12">
				<div class="row">
					...
					<table class="table table-hover" border="1"
						...
						<tbody>
							<c:forEach items="${view }" var="d">
								<tr>
									<th scope="row" style="text-align: center;">${d.dnum }</th>
									<!-- 글번호 -->
									<td style="text-align: center;"><a href="disc_detail?dnum=${d.dnum }">${d.dtitle}</a></td>
									
									<!--<fmt:parseDate value="${d.ddate }" var="dateValue" pattern="yyyyMMddHHmmss"/> 
				<td><fmt:formatDate value="${dateValue }" pattern="yyyy-MM-dd"/></td>
				
			</tr>-->				
									<td style="text-align: center;">${d.ddate }</td>
									<td style="text-align: center;">${d.dreadcnt }</td>
							</c:forEach>
							</tbody>
		<!-- 페이징처리 44444 -->
			<tr style="border-left: none; border-right: none; border-bottom: none">
							<td colspan="6" style="text-align: center;"><c:forEach
									...
									<c:choose>
										...
										<c:when test="${p != paging.nowPage }">
											<a
												href="disc_notice?nowPage=${p}&cntPerPage=${paging.cntPerPage}">${p}</a>
										</c:when>
									</c:choose>
								</c:forEach></td>
						</tr>			
